...
async function run() { 
  ...
  console.log('GenerateLink Data:', res.data); 
  console.log('GenerateLink Error:', res.error); 
  
  if (res.data && res.data.properties && res.data.properties.hashed_token) {
    const res2 = await admin.auth.verifyOtp({ token_hash: res.data.properties.hashed_token, type: 'magiclink' });
    console.log('VerifyOtp Data:', res2.data);
